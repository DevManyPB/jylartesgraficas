...
import { avisarAlSitio, reordenarProductos } from "@jyl/core/server";
...
/** Orden de los productos tras arrastrar — SPEC.md §6.7. */
export async function PUT(request: Request) {
  ...
  try {
    const aplicado = await reordenarProductos(datos.data.ids);
    if (!aplicado) {
